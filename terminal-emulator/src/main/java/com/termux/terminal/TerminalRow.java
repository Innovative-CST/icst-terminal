...
public final class TerminalRow {
    ...
    public void copyInterval(TerminalRow line, int sourceX1, int sourceX2, int destinationX) {
        ...
        for (int i = x1; i < x2; i++) {
            ...
            int codePoint = Character.isHighSurrogate(sourceChar) ? Character.toCodePoint(sourceChar, sourceChars[++i])
                    : sourceChar;
                ...
                if (currentColumn == column) {
                    while (newCharIndex < mSpaceUsed) {
                        ...
                        if (Character.isHighSurrogate(mText[newCharIndex])) {
                            if (WcWidth
                                    .width(Character.toCodePoint(mText[newCharIndex], mText[newCharIndex + 1])) <= 0) {
                                newCharIndex += 2;
                            } else {
                                break;
                            }
    ...
    public void setChar(int columnToSet, int codePoint, long style) {
        if (columnToSet < 0 || columnToSet >= mStyle.length)
            throw new IllegalArgumentException("TerminalRow.setChar(): columnToSet=" + columnToSet + ", codePoint="
        ...
        if (newIsCombining) {
            ...
            boolean overwritingWideCharInNextColumn = newCodePointDisplayWidth == 2
        ...
        if (newIsCombining) {
            int combiningCharsCount = WcWidth.zeroWidthCharsCount(mText, oldStartOfColumnIndex,
        ...
        if (oldCodePointDisplayWidth == 2 && newCodePointDisplayWidth == 1) {
            ...
            if (mSpaceUsed + 1 > text.length) {
                ...
                System.arraycopy(text, newNextColumnIndex, newText, newNextColumnIndex + 1,
                        mSpaceUsed - newNextColumnIndex);
                mText = text = newText;
            } else {
                System.arraycopy(text, newNextColumnIndex, text, newNextColumnIndex + 1,
                        mSpaceUsed - newNextColumnIndex);
            }
            ...
            if (columnToSet == mColumns - 1) {
                ...
                int newNextNextColumnIndex = newNextColumnIndex
                        + (Character.isHighSurrogate(mText[newNextColumnIndex]) ? 2 : 1);
                int nextLen = newNextNextColumnIndex - newNextColumnIndex;

                // Shift the array leftwards.
                System.arraycopy(text, newNextNextColumnIndex, text, newNextColumnIndex,
