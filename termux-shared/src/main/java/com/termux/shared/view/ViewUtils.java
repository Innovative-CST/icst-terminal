...
public class ViewUtils {
     ...
     */
    @Nullable public static Rect[] getWindowAndViewRects(View view, int statusBarHeight) {
        ...
        if (context instanceof AppCompatActivity) {
            ...
            isInMultiWindowMode = (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N)
                    && ((AppCompatActivity) context).isInMultiWindowMode();
        ...
        windowAvailableRect = new Rect(windowRect.left, windowRect.top + actionBarHeight, windowRect.right,
                windowRect.bottom);
        ...
        if (view_utils_logging_enabled) {
            ...
            Logger.logVerbose(LOG_TAG, "windowRect: " + toRectString(windowRect) + ", windowAvailableRect: "
                    + toRectString(windowAvailableRect));
        ...
        if (isInMultiWindowMode) {
            if (displayOrientation == Configuration.ORIENTATION_PORTRAIT) {
                ...
                if (statusBarHeight != windowRect.top) {
                    if (view_utils_logging_enabled)
                        Logger.logVerbose(LOG_TAG, "Window top does not equal statusBarHeight " + statusBarHeight
                                + " in multi-window portrait mode. Window is possibly bottom app in split screen mode. Adding windowRect.top "
                                + windowRect.top + " to viewTop.");
                    viewTop += windowRect.top;
                } else {
                    if (view_utils_logging_enabled)
                        Logger.logVerbose(LOG_TAG, "windowRect.top equals statusBarHeight " + statusBarHeight
                                + " in multi-window portrait mode. Window is possibly top app in split screen mode.");
                }
        ...
        if (displayOrientation == Configuration.ORIENTATION_LANDSCAPE && viewRight > windowAvailableRect.right) {
            if (view_utils_logging_enabled)
                Logger.logVerbose(LOG_TAG, "viewRight " + viewRight + " is greater than windowAvailableRect.right "
                        + windowAvailableRect.right
                        + " in landscape mode. Setting windowAvailableRect.right to viewRight since it may not include navbar height.");
    ...
    public static Point getDisplaySize(@NonNull Context context, boolean activitySize) {
        ...
        if (activitySize)
            windowMetrics = androidx.window.layout.WindowMetricsCalculator.getOrCreate()
    ...
    @Nullable public static Activity getActivity(Context context) {
