...
public class TermuxBootstrap {
    ...
    public static void setTermuxPackageManagerAndVariant(@Nullable String packageVariantName) {
        ...
        if (TERMUX_APP_PACKAGE_MANAGER == null) {
            throw new RuntimeException("Unsupported TERMUX_APP_PACKAGE_MANAGER \"" + packageManagerName
                    + "\" with variant \"" + packageVariantName + "\"");
        }
    ...
    public static void setTermuxPackageManagerAndVariantFromTermuxApp(@NonNull Context currentPackageContext) {
        ...
        if (packageVariantName != null) {
            ...
            Logger.logError(LOG_TAG,
                    "Failed to set TERMUX_APP_PACKAGE_VARIANT and TERMUX_APP_PACKAGE_MANAGER from the termux app");
        }
    }
    ...
    public static String getTermuxAppBuildConfigPackageVariantFromTermuxApp(@NonNull Context currentPackageContext) {
        try {
            return (String) TermuxUtils.getTermuxAppAPKBuildConfigClassField(currentPackageContext,
                    BUILD_CONFIG_FIELD_TERMUX_PACKAGE_VARIANT);
        } catch (Exception e) {
            Logger.logStackTraceWithMessage(LOG_TAG, "Failed to get \"" + BUILD_CONFIG_FIELD_TERMUX_PACKAGE_VARIANT
                    + "\" value from \"" + TERMUX_APP.BUILD_CONFIG_CLASS_NAME + "\" class", e);
            return null;
        }
    }
    ...
    public enum PackageManager {
        ...
        @Nullable public static PackageManager managerOf(String name) {
    ...
    public enum PackageVariant {
        ...
        @Nullable public static PackageVariant variantOf(String name) {
            if (name == null || name.isEmpty())
                return null;
