...
public class FileUtils {
     ...
     */
    @Nullable public static String normalizePath(String path) {
        if (path == null)
            return null;
    ...
    public static Error validateDirectoryFileEmptyOrOnlyContainsSpecificFiles(String label, String filePath,
        ...
        if (filePath == null || filePath.isEmpty())
            return FunctionErrno.ERRNO_NULL_OR_EMPTY_PARAMETER.getError(label + "file path",
        ...
        try {
            ...
            if (fileType != FileType.NO_EXIST && fileType != FileType.DIRECTORY) {
                return FileUtilsErrno.ERRNO_NON_DIRECTORY_FILE_FOUND.getError(label + "directory", filePath)
                        .setLabel(label + "directory");
            }
            ...
            return FileUtilsErrno.ERRNO_VALIDATE_DIRECTORY_EMPTY_OR_ONLY_CONTAINS_SPECIFIC_FILES_FAILED_WITH_EXCEPTION
                    .getError(e, label + "directory", filePath, e.getMessage());
        }
     ...
     */
    @NonNull public static FileType getFileType(final String filePath, final boolean followLinks) {
     ...
     */
    public static Error validateRegularFileExistenceAndPermissions(String label, final String filePath,
        ...
        label = (label == null || label.isEmpty() ? "" : label + " ");
        if (filePath == null || filePath.isEmpty())
            return FunctionErrno.ERRNO_NULL_OR_EMPTY_PARAMETER.getError(label + "regular file path",
        ...
        try {
            ...
            if (setPermissions && permissionsToCheck != null && fileType == FileType.REGULAR) {
                ...
                if (parentDirPath == null
                        || (isPathUnderParentDirPath && getFileType(parentDirPath, false) == FileType.DIRECTORY)) {
            ...
            return FileUtilsErrno.ERRNO_VALIDATE_FILE_EXISTENCE_AND_PERMISSIONS_FAILED_WITH_EXCEPTION.getError(e,
                    label + "file", filePath, e.getMessage());
        }
     ...
     */
    public static Error validateDirectoryFileExistenceAndPermissions(String label, final String filePath,
            final String parentDirPath, final boolean createDirectoryIfMissing,
            final String permissionsToCheck, final boolean setPermissions, final boolean setMissingPermissionsOnly,
            final boolean ignoreErrorsIfPathIsInParentDirPath, final boolean ignoreIfNotExecutable) {
        label = (label == null || label.isEmpty() ? "" : label + " ");
        if (filePath == null || filePath.isEmpty())
            return FunctionErrno.ERRNO_NULL_OR_EMPTY_PARAMETER.getError(label + "directory file path",
        ...
        try {
            ...
            if (createDirectoryIfMissing || setPermissions) {
                ...
                if (parentDirPath == null
                        || (isPathInParentDirPath && getFileType(parentDirPath, false) == FileType.DIRECTORY)) {
                    ...
                    if (createDirectoryIfMissing && fileType == FileType.NO_EXIST) {
                        ...
                        if (!result && fileType != FileType.DIRECTORY)
                            return FileUtilsErrno.ERRNO_CREATING_FILE_FAILED.getError(label + "directory file",
                                    filePath);
                    }
            ...
            if (parentDirPath == null || !isPathInParentDirPath || !ignoreErrorsIfPathIsInParentDirPath) {
                ...
                if (permissionsToCheck != null) {
                    // Check if permissions are missing
                    return checkMissingFilePermissions(label + "directory", filePath, permissionsToCheck,
                            ignoreIfNotExecutable);
                }
            }
        } catch (Exception e) {
            return FileUtilsErrno.ERRNO_VALIDATE_DIRECTORY_EXISTENCE_AND_PERMISSIONS_FAILED_WITH_EXCEPTION.getError(e,
                    label + "directory file", filePath, e.getMessage());
        }
    ...
    public static Error createRegularFile(String label, final String filePath,
        ...
        try {
            ...
            return FileUtilsErrno.ERRNO_CREATING_FILE_FAILED_WITH_EXCEPTION.getError(e, label + "regular file",
    ...
    public static Error createSymlinkFile(String label, final String targetFilePath, final String destFilePath,
        ...
        if (targetFilePath == null || targetFilePath.isEmpty())
            return FunctionErrno.ERRNO_NULL_OR_EMPTY_PARAMETER.getError(label + "target file path",
                    "createSymlinkFile");
        if (destFilePath == null || destFilePath.isEmpty())
            return FunctionErrno.ERRNO_NULL_OR_EMPTY_PARAMETER.getError(label + "destination file path",
                    "createSymlinkFile");
        ...
        try {
            ...
            if (targetFileType == FileType.NO_EXIST) {
                ...
                if (!allowDangling) {
                    ...
                    return FileUtilsErrno.ERRNO_FILE_NOT_FOUND_AT_PATH.getError(label, targetFileAbsolutePath)
                            .setLabel(label);
                }
            }
            ...
            if (destFileType != FileType.NO_EXIST) {
                ...
                if (overwriteOnlyIfDestIsASymlink && destFileType != FileType.SYMLINK)
                    return FileUtilsErrno.ERRNO_CANNOT_OVERWRITE_A_NON_SYMLINK_FILE_TYPE.getError(label + " file",
                            destFilePath, targetFilePath, destFileType.getName());
            ...
            Logger.logVerbose(LOG_TAG, "Creating " + label + "symlink file at path \"" + destFilePath + "\" to \""
                    + targetFilePath + "\"");
            Os.symlink(targetFilePath, destFilePath);
        } catch (Exception e) {
            return FileUtilsErrno.ERRNO_CREATING_SYMLINK_FILE_FAILED_WITH_EXCEPTION.getError(e, label + "symlink file",
                    destFilePath, targetFilePath, e.getMessage());
        }
     ...
     */
    public static Error copyRegularFile(final String label, final String srcFilePath, final String destFilePath,
     ...
     */
    public static Error moveRegularFile(final String label, final String srcFilePath, final String destFilePath,
            final boolean ignoreNonExistentSrcFile) {
        return copyOrMoveFile(label, srcFilePath, destFilePath,
     ...
     * @return Returns the {@code error} if copy was not successful, otherwise {@code null}.
     */
    public static Error copyDirectoryFile(final String label, final String srcFilePath, final String destFilePath,
            final boolean ignoreNonExistentSrcFile) {
        return copyOrMoveFile(label, srcFilePath, destFilePath,
     ...
     * @return Returns the {@code error} if move was not successful, otherwise {@code null}.
     */
    public static Error moveDirectoryFile(final String label, final String srcFilePath, final String destFilePath,
            final boolean ignoreNonExistentSrcFile) {
        return copyOrMoveFile(label, srcFilePath, destFilePath,
     ...
     * @return Returns the {@code error} if copy was not successful, otherwise {@code null}.
     */
    public static Error copySymlinkFile(final String label, final String srcFilePath, final String destFilePath,
            final boolean ignoreNonExistentSrcFile) {
        return copyOrMoveFile(label, srcFilePath, destFilePath,
     ...
     * @return Returns the {@code error} if move was not successful, otherwise {@code null}.
     */
    public static Error moveSymlinkFile(final String label, final String srcFilePath, final String destFilePath,
            final boolean ignoreNonExistentSrcFile) {
        return copyOrMoveFile(label, srcFilePath, destFilePath,
     ...
     * @return Returns the {@code error} if copy was not successful, otherwise {@code null}.
     */
    public static Error copyFile(final String label, final String srcFilePath, final String destFilePath,
            final boolean ignoreNonExistentSrcFile) {
        return copyOrMoveFile(label, srcFilePath, destFilePath,
     ...
     * @return Returns the {@code error} if move was not successful, otherwise {@code null}.
     */
    public static Error moveFile(final String label, final String srcFilePath, final String destFilePath,
            final boolean ignoreNonExistentSrcFile) {
        return copyOrMoveFile(label, srcFilePath, destFilePath,
    ...
    public static Error copyOrMoveFile(String label, final String srcFilePath, final String destFilePath,
        ...
        try {
            Logger.logVerbose(LOG_TAG, mode + " " + label + "source file from \"" + srcFilePath + "\" to destination \""
                    + destFilePath + "\"");
            ...
            if ((allowedFileTypeFlags & srcFileType.getValue()) <= 0)
                return FileUtilsErrno.ERRNO_FILE_NOT_AN_ALLOWED_FILE_TYPE.getError(
                        label + "source file meant to be " + modePast, srcFilePath,
            ...
            if (srcFileCanonicalPath.equals(destFileCanonicalPath))
                return FileUtilsErrno.ERRNO_COPYING_OR_MOVING_FILE_TO_SAME_PATH
                        .getError(mode + " " + label + "source file", srcFilePath, destFilePath);
            ...
            if (destFileType != FileType.NO_EXIST) {
                ...
                if (overwriteOnlyIfDestSameFileTypeAsSrc && destFileType != srcFileType)
                    return FileUtilsErrno.ERRNO_CANNOT_OVERWRITE_A_DIFFERENT_FILE_TYPE.getError(label + "source file",
                            mode.toLowerCase(), srcFilePath, destFilePath, destFileType.getName(),
                            srcFileType.getName());
            ...
            if (moveFile) {
                ...
                if (!srcFile.renameTo(destFile)) {
                    ...
                    if (srcFileType == FileType.DIRECTORY
                            && destFileCanonicalPath.startsWith(srcFileCanonicalPath + File.separator))
                        return FileUtilsErrno.ERRNO_CANNOT_MOVE_DIRECTORY_TO_SUB_DIRECTORY_OF_ITSELF
                                .getError(label + "source directory", srcFilePath, destFilePath);
            ...
            if (copyFile) {
                ...
                if (srcFileType == FileType.DIRECTORY) {
                    ...
                    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
                        java.nio.file.Files.copy(srcFile.toPath(), destFile.toPath(), LinkOption.NOFOLLOW_LINKS,
            ...
            return FileUtilsErrno.ERRNO_COPYING_OR_MOVING_FILE_FAILED_WITH_EXCEPTION.getError(e,
                    mode + " " + label + "file", srcFilePath, destFilePath, e.getMessage());
        }
     ...
     */
    public static Error deleteFile(String label, final String filePath, final boolean ignoreNonExistentFile,
            final boolean ignoreWrongFileType, int allowedFileTypeFlags) {
        label = (label == null || label.isEmpty() ? "" : label + " ");
        if (filePath == null || filePath.isEmpty())
        ...
        try {
            ...
            Logger.logVerbose(LOG_TAG, "Processing delete of " + label + "file at path \"" + filePath + "\" of type \""
                    + fileType.getName() + "\"");
            ...
            if ((allowedFileTypeFlags & fileType.getValue()) <= 0) {
                ...
                if (ignoreWrongFileType) {
                    Logger.logVerbose(LOG_TAG,
                            "Ignoring deletion of " + label + "file at path \"" + filePath + "\" of type \""
                                    + fileType.getName() + "\" not matching allowed file types: "
                                    + FileTypes.convertFileTypeFlagsToNamesString(allowedFileTypeFlags));
                    return null;
                }

                // Else return with error
                return FileUtilsErrno.ERRNO_FILE_NOT_AN_ALLOWED_FILE_TYPE.getError(label + "file meant to be deleted",
                        filePath, fileType.getName(),
                        FileTypes.convertFileTypeFlagsToNamesString(allowedFileTypeFlags));
            }

            Logger.logVerbose(LOG_TAG, "Deleting " + label + "file at path \"" + filePath + "\"");

            Error error = deleteRecursively(label, file);
            if (error != null)
                return error;
            ...
            fileType = getFileType(filePath, false);
            if (fileType != FileType.NO_EXIST)
                return FileUtilsErrno.ERRNO_FILE_STILL_EXISTS_AFTER_DELETING
                        .getError(label + "file meant to be deleted", filePath);
        } catch (Exception e) {
            return FileUtilsErrno.ERRNO_DELETING_FILE_FAILED_WITH_EXCEPTION.getError(e, label + "file", filePath,
                    e.getMessage());
        }
    ...
    public static Error deleteRecursively(String label, File file) {
        ...
        if (file.isDirectory()) {
            ...
            if (children != null) {
                for (File child : children) {
                    deleteRecursively(label, child);
                }
            }
        }

        if (!file.delete()) {
            IOException e = new IOException("Failed to delete: " + file.getAbsolutePath());
            return FileUtilsErrno.ERRNO_CLEARING_DIRECTORY_FAILED_WITH_EXCEPTION.getError(e, label + "directory",
    ...
    public static Error clearDirectory(String label, final String filePath) {
        ...
        try {
            ...
            if (fileType == FileType.DIRECTORY) {
                error = deleteRecursively(label, file);
                if (error != null)
                    return error;
     ...
     */
    public static Error deleteFilesOlderThanXDays(String label, final String filePath, final IOFileFilter dirFilter,
            int days, final boolean ignoreNonExistentFile, int allowedFileTypeFlags) {
        label = (label == null || label.isEmpty() ? "" : label + " ");
        if (filePath == null || filePath.isEmpty())
            return FunctionErrno.ERRNO_NULL_OR_EMPTY_PARAMETER.getError(label + "file path",
                    "deleteFilesOlderThanXDays");
        if (days < 0)
            return FunctionErrno.ERRNO_INVALID_PARAMETER.getError(label + "days", "deleteFilesOlderThanXDays",
                    " It must be >= 0.");

        Error error;

        try {
            Logger.logVerbose(LOG_TAG, "Deleting files under " + label + "directory at path \"" + filePath
            ...
            Iterator<File> filesToDelete = org.apache.commons.io.FileUtils.iterateFiles(file,
                    new AgeFileFilter(calendar.getTime()), dirFilter);
            while (filesToDelete.hasNext()) {
                File subFile = filesToDelete.next();
                error = deleteFile(label + " directory sub", subFile.getAbsolutePath(), true, true,
                        allowedFileTypeFlags);
                if (error != null)
                    return error;
            }
        } catch (Exception e) {
            return FileUtilsErrno.ERRNO_DELETING_FILES_OLDER_THAN_X_DAYS_FAILED_WITH_EXCEPTION.getError(e,
                    label + "directory", filePath, days, e.getMessage());
        }
     ...
     */
    public static Error readTextFromFile(String label, final String filePath, Charset charset,
            @NonNull final StringBuilder dataStringBuilder, final boolean ignoreNonExistentFile) {
        label = (label == null || label.isEmpty() ? "" : label + " ");
        if (filePath == null || filePath.isEmpty())
        ...
        try {
            ...
            Logger.logVerbose(LOG_TAG, Logger.getMultiLineLogStringEntry("String", DataUtils.getTruncatedCommandOutput(
                    dataStringBuilder.toString(), Logger.LOGGER_ENTRY_MAX_SAFE_PAYLOAD, true, false, true), "-"));
        } catch (Exception e) {
            return FileUtilsErrno.ERRNO_READING_TEXT_FROM_FILE_FAILED_WITH_EXCEPTION.getError(e, label + "file",
                    filePath, e.getMessage());
        } finally {
     ...
     * @return Returns the {@code error} if reading was not successful, otherwise {@code null}.
     */
    @NonNull public static <T extends Serializable> ReadSerializableObjectResult readSerializableObjectFromFile(String label,
            final String filePath, Class<T> readObjectType, final boolean ignoreNonExistentFile) {
        label = (label == null || label.isEmpty() ? "" : label + " ");
        if (filePath == null || filePath.isEmpty())
            return new ReadSerializableObjectResult(FunctionErrno.ERRNO_NULL_OR_EMPTY_PARAMETER
                    .getError(label + "file path", "readSerializableObjectFromFile"), null);
        ...
        // If file exists but not a regular file
        if (fileType != FileType.NO_EXIST && fileType != FileType.REGULAR) {
            return new ReadSerializableObjectResult(FileUtilsErrno.ERRNO_NON_REGULAR_FILE_FOUND
                    .getError(label + "file", filePath).setLabel(label + "file"), null);
        }
        ...
        if (fileType == FileType.NO_EXIST) {
            ...
            // Else return with error
                ...
                return new ReadSerializableObjectResult(
                        FileUtilsErrno.ERRNO_FILE_NOT_FOUND_AT_PATH.getError(label, filePath).setLabel(label), null);
            }
        }
        ...
        try {
            ...
            return new ReadSerializableObjectResult(
                    FileUtilsErrno.ERRNO_READING_SERIALIZABLE_OBJECT_TO_FILE_FAILED_WITH_EXCEPTION.getError(e,
                            label + "file", filePath, e.getMessage()),
                    null);
        } finally {
            closeCloseable(fileInputStream);
     ...
     */
    public static Error writeTextToFile(String label, final String filePath, Charset charset, final String dataString,
        ...
        Logger.logVerbose(LOG_TAG,
                Logger.getMultiLineLogStringEntry("Writing text to " + label + "file at path \"" + filePath + "\"",
                        DataUtils.getTruncatedCommandOutput(dataString, Logger.LOGGER_ENTRY_MAX_SAFE_PAYLOAD, true,
                                false, true),
                        "-"));
        ...
        try {
            ...
            return FileUtilsErrno.ERRNO_WRITING_TEXT_TO_FILE_FAILED_WITH_EXCEPTION.getError(e, label + "file", filePath,
                    e.getMessage());
        } finally {
     ...
     * @return Returns the {@code error} if writing was not successful, otherwise {@code null}.
     */
    public static <T extends Serializable> Error writeSerializableObjectToFile(String label, final String filePath,
            final T serializableObject) {
        label = (label == null || label.isEmpty() ? "" : label + " ");
        if (filePath == null || filePath.isEmpty())
            return FunctionErrno.ERRNO_NULL_OR_EMPTY_PARAMETER.getError(label + "file path",
        ...
        try {
            ...
            return FileUtilsErrno.ERRNO_WRITING_SERIALIZABLE_OBJECT_TO_FILE_FAILED_WITH_EXCEPTION.getError(e,
                    label + "file", filePath, e.getMessage());
        } finally {
            closeCloseable(fileOutputStream);
    ...
    public static Error isCharsetSupported(final Charset charset) {
        ...
        try {
            ...
            return FileUtilsErrno.ERRNO_CHECKING_IF_CHARSET_SUPPORTED_FAILED.getError(e, charset.name(),
                    e.getMessage());
        }
     ...
     */
    public static Error checkMissingFilePermissions(final String filePath, final String permissionsToCheck,
            final boolean ignoreIfNotExecutable) {
     ...
     * @return Returns the {@code error} if validating permissions failed, otherwise {@code null}.
     */
    public static Error checkMissingFilePermissions(String label, final String filePath,
            final String permissionsToCheck, final boolean ignoreIfNotExecutable) {
        label = (label == null || label.isEmpty() ? "" : label + " ");
        if (filePath == null || filePath.isEmpty())
            return FunctionErrno.ERRNO_NULL_OR_EMPTY_PARAMETER.getError(label + "file path",
