...
public class LocalServerSocket implements Closeable {
    ...
    public synchronized Error start() {
        ...
        if (backlog <= 0) {
            return LocalSocketErrno.ERRNO_SERVER_SOCKET_BACKLOG_INVALID.getError(mLocalSocketRunConfig.getTitle(),
                    backlog);
        }
        ...
        if (!mLocalSocketRunConfig.isAbstractNamespaceSocket()) {
            if (!path.startsWith("/"))
                return LocalSocketErrno.ERRNO_SERVER_SOCKET_PATH_NOT_ABSOLUTE.getError(mLocalSocketRunConfig.getTitle(),
                        path);
        ...
        if (result == null || result.retval != 0) {
            return LocalSocketErrno.ERRNO_CREATE_SERVER_SOCKET_FAILED.getError(mLocalSocketRunConfig.getTitle(),
    ...
    public synchronized Error closeServerSocket(boolean logErrorMessage) {
        ...
        try {
            ...
            Error error = LocalSocketErrno.ERRNO_CLOSE_SERVER_SOCKET_FAILED_WITH_EXCEPTION.getError(e,
    ...
    private Error deleteServerSocketFile() {
        if (!mLocalSocketRunConfig.isAbstractNamespaceSocket())
            return FileUtils.deleteSocketFile(mLocalSocketRunConfig.getTitle() + " server socket file",
                    mLocalSocketRunConfig.getPath(), true);
        else
            return null;
    }
        ...
        while (true) {
            ...
            if (result == null || result.retval != 0) {
                mLocalSocketManager.onError(
                        LocalSocketErrno.ERRNO_ACCEPT_CLIENT_SOCKET_FAILED.getError(mLocalSocketRunConfig.getTitle(),
                                JniResult.getErrorString(result)));
                continue;
            }
            ...
            if (clientFD < 0) {
                mLocalSocketManager.onError(
                        LocalSocketErrno.ERRNO_CLIENT_SOCKET_FD_INVALID.getError(clientFD,
                                mLocalSocketRunConfig.getTitle()));
                continue;
            }

            PeerCred peerCred = new PeerCred();
            result = LocalSocketManager.getPeerCred(mLocalSocketRunConfig.getLogTitle() + " (client)", clientFD,
                    peerCred);
            if (result == null || result.retval != 0) {
                mLocalSocketManager.onError(
                        LocalSocketErrno.ERRNO_GET_CLIENT_SOCKET_PEER_UID_FAILED
            ...
            if (peerUid < 0) {
                mLocalSocketManager.onError(
                        LocalSocketErrno.ERRNO_CLIENT_SOCKET_PEER_UID_INVALID.getError(peerUid,
                                mLocalSocketRunConfig.getTitle()));
                LocalClientSocket.closeClientSocket(mLocalSocketManager, clientFD);
                continue;
            }
                ...
                while (!Thread.currentThread().isInterrupted()) {
                    ...
                    try {
                        ...
                        mLocalSocketManager.onError(clientSocket,
                                LocalSocketErrno.ERRNO_CLIENT_SOCKET_LISTENER_FAILED_WITH_EXCEPTION.getError(t,
                                        mLocalSocketRunConfig.getTitle(), t.getMessage()));
                        if (clientSocket != null)
                            clientSocket.closeClientSocket(true);
                    }
                }
