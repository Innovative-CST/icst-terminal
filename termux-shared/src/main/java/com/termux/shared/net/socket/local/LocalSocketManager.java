...
public class LocalSocketManager {
    ...
    public synchronized Error start() {
        ...
        if (!localSocketLibraryLoaded) {
            try {
                ...
                Error error = LocalSocketErrno.ERRNO_START_LOCAL_SOCKET_LIB_LOAD_FAILED_WITH_EXCEPTION.getError(t,
                        LOCAL_SOCKET_LIBRARY, t.getMessage());
    ...
    */
     ...
     * fd.
     */
    @Nullable public static JniResult createServerSocket(@NonNull String serverTitle, @NonNull byte[] path, int backlog) {
        try {
     ...
     */
    @Nullable public static JniResult closeSocket(@NonNull String serverTitle, int fd) {
        try {
     ...
     * fd.
     */
    @Nullable public static JniResult accept(@NonNull String serverTitle, int fd) {
        try {
     ...
     */
    @Nullable public static JniResult read(@NonNull String serverTitle, int fd, @NonNull byte[] data, long deadline) {
        try {
     ...
     * will be 0.
     */
    @Nullable public static JniResult send(@NonNull String serverTitle, int fd, @NonNull byte[] data, long deadline) {
        try {
     ...
     */
    @Nullable public static JniResult available(@NonNull String serverTitle, int fd) {
        try {
     ...
     * {@link JniResult#retval} will be 0.
     */
    @Nullable public static JniResult setSocketReadTimeout(@NonNull String serverTitle, int fd, int timeout) {
        try {
     ...
     * {@link JniResult#retval} will be 0.
     */
    @Nullable public static JniResult setSocketSendTimeout(@NonNull String serverTitle, int fd, int timeout) {
        try {
     ...
     * {@link JniResult#retval} will be 0.
     */
    @Nullable public static JniResult getPeerCred(@NonNull String serverTitle, int fd, PeerCred peerCred) {
        try {
    ...
    protected Thread.UncaughtExceptionHandler getLocalSocketManagerClientThreadUEHOrDefault() {
        Thread.UncaughtExceptionHandler uncaughtExceptionHandler = mLocalSocketManagerClient
                .getLocalSocketManagerClientThreadUEH(this);
        if (uncaughtExceptionHandler == null)
            uncaughtExceptionHandler = (t, e) -> Logger.logStackTraceWithMessage(LOG_TAG,
                    "Uncaught exception for " + t + " in " + mLocalSocketRunConfig.getTitle() + " server", e);
    ...
    @Nullable private static native JniResult createServerSocketNative(@NonNull String serverTitle, @NonNull byte[] path,
            int backlog);
    ...
    @Nullable private static native JniResult readNative(@NonNull String serverTitle, int fd, @NonNull byte[] data,
            long deadline);

    @Nullable private static native JniResult sendNative(@NonNull String serverTitle, int fd, @NonNull byte[] data,
            long deadline);
