...
public class StreamGobbler extends Thread {
    ...
    public void run() {
        ...
        try {
            ...
            while ((line = reader.readLine()) != null) {
                if (loggingEnabled)
                    Logger.logVerboseForce(defaultLogTag + "Command",
                            String.format(Locale.ENGLISH, "[%s] %s", shell, line)); // This will get truncated by LOGGER_ENTRY_MAX_LEN, likely 4KB
    ...
    void conditionalJoin() throws InterruptedException {
        if (calledOnClose)
            return; // deadlock from callback, we're inside exit procedure
        if (Thread.currentThread() == this)
            return; // can't join self
        join();
    }
}
