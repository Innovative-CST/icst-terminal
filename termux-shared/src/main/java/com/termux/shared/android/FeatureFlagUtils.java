...
public class FeatureFlagUtils {
     ...
     */
    @Nullable public static Boolean featureFlagExists(@NonNull String feature) {
     ...
     */
    @NonNull public static FeatureFlagValue getFeatureFlagValueString(@NonNull Context context, @NonNull String feature) {
     ...
     * {@code null} if an exception is raised.
     */
    @Nullable public static Boolean isFeatureEnabled(@NonNull Context context, @NonNull String feature) {
        ReflectionUtils.bypassHiddenAPIReflectionRestrictions();
        try {
            @SuppressLint("PrivateApi")
            Class<?> clazz = Class.forName(FEATURE_FLAGS_CLASS);
            Method isFeatureEnabledMethod = ReflectionUtils.getDeclaredMethod(clazz, "isEnabled", Context.class,
                    String.class);
            ...
            // ClassCastException may be thrown
            Logger.logStackTraceWithMessage(LOG_TAG, "Failed to check if feature flag \"" + feature + "\" is enabled",
                    e);
            return null;
        }
    }
