...
public class UserUtils {
     ...
     */
    @Nullable public static String getNameForUid(@NonNull Context context, int uid) {
    ...
    @Nullable public static String getNameForUidFromPackageManager(@NonNull Context context, int uid) {
        ...
        try {
            ...
            Logger.logStackTraceWithMessage(LOG_TAG, "Failed to get name for uid \"" + uid + "\" from package manager",
                    e);
            return null;
        }
    }
     ...
     * @return Returns the user name if found, otherwise {@code null}.
     */
    @Nullable public static String getNameForUidFromLibcore(int uid) {
        if (uid < 0)
            return null;
        ...
        try {
            ...
            try {
                ...
                Logger.logStackTraceWithMessage(LOG_TAG,
                        "Failed to get \"os\" field value for " + libcoreClassName + " class", e);
                return null;
            }
            ...
            if (clazz == null) {
                Logger.logError(LOG_TAG,
                        "Failed to find super class ForwardingOs from object of class " + os.getClass().getName());
                return null;
            }
            ...
            try {
                ...
                Logger.logStackTraceWithMessage(LOG_TAG,
                        "Failed to invoke getpwuid() method of " + clazz.getName() + " class", e);
                return null;
            }
            ...
            try {
                ...
                // ClassCastException may be thrown
                Logger.logStackTraceWithMessage(LOG_TAG,
                        "Failed to get \"pw_name\" field value for " + clazz.getName() + " class", e);
                return null;
            }
        } catch (Exception e) {
