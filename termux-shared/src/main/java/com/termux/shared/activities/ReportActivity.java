...
public class ReportActivity extends AppCompatActivity {
    ...
    private static final String ACTION_DELETE_REPORT_INFO_OBJECT_FILE = CLASS_NAME
    ...
    private void updateUI() {
        ...
        if (mBundle.containsKey(EXTRA_REPORT_INFO_OBJECT_FILE_PATH)) {
            ...
            Logger.logVerbose(LOG_TAG, ReportInfo.class.getSimpleName()
                    + " serialized object will be read from file at path \"" + mReportInfoFilePath + "\"");
            if (mReportInfoFilePath != null) {
                try {
                    FileUtils.ReadSerializableObjectResult result = FileUtils.readSerializableObjectFromFile(
                            ReportInfo.class.getSimpleName(), mReportInfoFilePath, ReportInfo.class, false);
                    ...
                    Logger.logStackTraceWithMessage(LOG_TAG, "Failure while getting " + ReportInfo.class.getSimpleName()
                            + " serialized object from file at path \"" + mReportInfoFilePath + "\"", e);
                }
            }
        } else {
    ...
    private void generateReportActivityMarkdownString() {
        ...
        if (reportMarkdownStringSize > ACTIVITY_TEXT_SIZE_LIMIT_IN_BYTES) {
            Logger.logVerbose(LOG_TAG, mReportInfo.reportTitle + " report string size " + reportMarkdownStringSize
                    + " is greater than " + ACTIVITY_TEXT_SIZE_LIMIT_IN_BYTES + " and will be truncated");
            reportString.append(DataUtils.getTruncatedCommandOutput(reportMarkdownString,
                    ACTIVITY_TEXT_SIZE_LIMIT_IN_BYTES, true, false, true));
            truncated = true;
        } else {
        ...
        if (reportStringSize > ACTIVITY_TEXT_SIZE_LIMIT_IN_BYTES) {
            ...
            Logger.logVerbose(LOG_TAG, mReportInfo.reportTitle + " report string total size " + reportStringSize
                    + " is greater than " + ACTIVITY_TEXT_SIZE_LIMIT_IN_BYTES + " and will be truncated");
            mReportActivityMarkdownString = this.getString(R.string.msg_report_truncated) +
                    DataUtils.getTruncatedCommandOutput(reportString.toString(), ACTIVITY_TEXT_SIZE_LIMIT_IN_BYTES,
                            true, false, false);
        } else if (truncated) {
     ...
     */
    @NonNull public static NewInstanceResult newInstance(@NonNull final Context context, @NonNull final ReportInfo reportInfo) {
        ...
        if (size > DataUtils.TRANSACTION_SIZE_LIMIT_IN_BYTES) {
            ...
            String reportInfoFilePath = reportInfoDirectoryPath + "/" + CACHE_FILE_BASENAME_PREFIX
                    + reportInfo.reportTimestamp;
            Logger.logVerbose(LOG_TAG,
                    reportInfo.reportTitle + " " + ReportInfo.class.getSimpleName() + " serialized object size " + size
                            + " is greater than " + DataUtils.TRANSACTION_SIZE_LIMIT_IN_BYTES
                            + " and it will be written to file at path \"" + reportInfoFilePath + "\"");
            Error error = FileUtils.writeSerializableObjectToFile(ReportInfo.class.getSimpleName(), reportInfoFilePath,
                    reportInfo);
            if (error != null) {
    ...
    private static Intent createContentIntent(@NonNull final Context context, final ReportInfo reportInfo,
            final String reportInfoFilePath) {
        ...
        intent.setFlags(
                Intent.FLAG_ACTIVITY_SINGLE_TOP | Intent.FLAG_ACTIVITY_NEW_TASK | Intent.FLAG_ACTIVITY_NEW_DOCUMENT);
        return intent;
    }
    ...
    @NotNull private static String getReportInfoDirectoryPath(Context context) {
    ...
    private static void deleteReportInfoFile(Context context, String reportInfoFilePath) {
        ...
        if (!reportInfoFilePath.equals(reportInfoDirectoryPath)
                && reportInfoFilePath.startsWith(reportInfoDirectoryPath + "/")) {
            Logger.logVerbose(LOG_TAG, "Deleting " + ReportInfo.class.getSimpleName()
                    + " serialized object file at path \"" + reportInfoFilePath + "\"");
            ...
            Logger.logError(LOG_TAG,
                    "Not deleting " + ReportInfo.class.getSimpleName() + " serialized object file at path \""
                            + reportInfoFilePath + "\" since its not under \"" + reportInfoDirectoryPath + "\"");
        }
    }
     ...
     */
    public static Error deleteReportInfoFilesOlderThanXDays(@NonNull final Context context, int days,
    ...
    private static Error deleteReportInfoFilesOlderThanXDaysInner(@NonNull final Context context, int days) {
        ...
        String reportInfoDirectoryPath = getReportInfoDirectoryPath(context);
        Logger.logVerbose(LOG_TAG,
                "Deleting " + ReportInfo.class.getSimpleName() + " serialized object files under directory path \""
                        + reportInfoDirectoryPath + "\" older than " + days + " days");
        return FileUtils.deleteFilesOlderThanXDays(ReportInfo.class.getSimpleName(), reportInfoDirectoryPath, null,
                days, true, FileType.REGULAR.getValue());
    }
